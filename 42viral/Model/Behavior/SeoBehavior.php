...
class SeoBehavior extends ModelBehavior
{
    ...
    {        
        ...
        if(!isset($model->data[$model->name]['id'])) {
            $fieldToConvert = $model->data[$model->name][$this->settings[$model->name]['convert']];

            $baseSlug = $this->__baseSlug($model, $fieldToConvert);
     ...
     * Returns an SEO/URL freindly version of a given string
     * @param object $model
     * @param string $baseSlug the title of a content record as entered by the user
     * @return string the reformatted version of the input string
     * @access private 
     */
